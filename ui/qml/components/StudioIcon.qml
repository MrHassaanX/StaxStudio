...
Canvas {
    ...
    onPaint: {
        ...
        if (name === "add") {
            ...
            ctx.beginPath(); ctx.ellipse(w * .5, h * .5, w * .36, h * .23, 0, 0, Math.PI * 2); ctx.stroke(); ctx.beginPath(); ctx.arc(w * .5, h * .5, 2.1, 0, Math.PI * 2); ctx.fill()
            ...
            else { ctx.moveTo(w * .37, h * .45); ctx.arc(w * .56, h * .45, w * .19, Math.PI, 0) }
            ctx.stroke()
        } else if (name === "mute") {
